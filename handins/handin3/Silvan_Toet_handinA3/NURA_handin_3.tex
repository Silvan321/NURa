...
For each halo-mass bin, I fit the same parametric profile
𝑛
(
𝑥
;
...
n(x;a,b,c) separately. This yields five different best-fit parameter sets
(
𝑎
,
𝑏
...
(a,b,c), one for each mass bin. The normalization
𝐴
A is not an independent fit parameter, but is recalculated from
𝑎
,
𝑏
,
𝑐
a,b,c for each trial model. Because the radial coordinate is normalized by the virial radius, the model describes the dimensionless shape of the satellite distribution, allowing that shape to vary with halo mass.
